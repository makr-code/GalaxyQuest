...
CREATE TABLE IF NOT EXISTS npc_factions (
    ...
    faction_type ENUM('military','trade','science','pirate','ancient','envoy') NOT NULL,
...
-- ─── Seed: Prolog herald factions (envoys) ────────────────────────────────────
-- These six factions correspond to the narrative prolog faction selection in the
-- registration flow.  faction_type='envoy' distinguishes them from the classic
-- NPC diplomacy factions so the diplomacy list can filter them if needed.
INSERT IGNORE INTO npc_factions
    (code, name, description, faction_type, aggression, trade_willingness,
     base_diplomacy, power_level, home_galaxy_min, home_galaxy_max, color, icon)
VALUES
