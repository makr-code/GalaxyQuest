...
function parse_options(array $argv): array {
    ...
    foreach ($argv as $arg) {
        ...
        if ($arg === '--package-debug-comments') {
            $opts['package_debug_comments'] = true;
            continue;
        }
...
function build_boot_bundles_gzip(string $root, array $options): void {
    ...
    $matches = [1 => []];
    preg_match_all('/["\'](js\/[^"\']+\.js(?:\?[^"\']*)?)["\']/i', $index, $indexMatches);
    if (!empty($indexMatches[1])) {
        $matches[1] = array_merge($matches[1], $indexMatches[1]);
    }

    $bootManifestPath = $root . DIRECTORY_SEPARATOR . 'js' . DIRECTORY_SEPARATOR . 'runtime' . DIRECTORY_SEPARATOR . 'boot-manifest.js';
    if (is_file($bootManifestPath)) {
        $manifest = file_get_contents($bootManifestPath);
        if ($manifest !== false) {
            preg_match_all('/["\'](js\/[^"\']+\.js(?:\?[^"\']*)?)["\']/i', $manifest, $manifestMatches);
            if (!empty($manifestMatches[1])) {
                $matches[1] = array_merge($matches[1], $manifestMatches[1]);
            }
        }
    }

    if (empty($matches[1])) {
        fwrite(STDOUT, "[package] no boot JS references found in index/boot-manifest.\n");
        return;
    }
    ...
    $totalIncluded = 0;
    $totalGzBytes = 0;
    $packageDebugComments = !empty($options['package_debug_comments']);
    $packageUseUglify = !empty($options['uglify']);
    ...
    foreach ($themeKeys as $theme) {
        ...
        foreach ($themePaths as $rel) {
            ...
            $processed = $packageUseUglify
                ? uglify_js_aggressive($content, $abs)
                : minify_js_safe($content);
            $processed = rtrim(str_replace(["\r\n", "\r"], "\n", $processed));
            if ($processed === '') {
                continue;
            }

            if ($packageDebugComments) {
                $parts[] = "\n/* ---- {$rel} ---- */\n";
            }
            $parts[] = "\n;\n" . $processed . "\n";
            $included++;
