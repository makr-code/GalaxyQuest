...
switch ($action) {
	...
	case 'iron_fleet_compose':
		...
		// ── Direct NPC character chat (file-backed per-session history) ───────────
	case 'chat_npc':
		only_method('POST');
		verify_csrf();
		$body = get_json_body();
		...
		// Load this session's messages from disk and build the message array.
		...
		// Open questions → 3 stanced options (zustimmend/skeptisch/ablehnend).
		// Yes/no questions → ["Ja","Nein"]; is_yes_no=true.
		// Falls back to empty array on any LLM or parse failure (non-blocking).
		$suggestionsMessages = [
		[
		'role'    => 'system',
		'content' => 'Deine Aufgabe: Analysiere die folgende NPC-Aussage und generiere Antwortoptionen für den Spieler. Antworte NUR mit einem gültigen JSON-Objekt ohne Markdown: {"is_yes_no":bool,"suggestions":["...","...","..."]}. Regel: Wenn die NPC-Aussage eine Ja/Nein-Frage ist, setze is_yes_no=true und suggestions=["Ja","Nein"]. Andernfalls is_yes_no=false und liefere genau 3 kurze Antwortoptionen (max. 80 Zeichen je) mit unterschiedlicher Haltung: eine zustimmende, eine neutrale oder skeptische und eine ablehnende. Formuliere die Optionen in der Sprache des Gesprächs aus Spielerperspektive.',
		],
		[
		'role'    => 'user',
		'content' => $npcReply,
