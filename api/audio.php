...
/**
 * Audio catalog API
 * GET /api/audio.php?action=list       – list music tracks
 * GET /api/audio.php?action=list_sfx   – list SFX/teaser files
 */
require_once __DIR__ . '/helpers.php';

$action = (string)($_GET['action'] ?? 'list');

if ($action !== 'list' && $action !== 'list_sfx') {
...
// action === 'list_sfx'
$sfxDir = realpath(__DIR__ . '/../sfx');
if ($sfxDir === false || !is_dir($sfxDir)) {
    json_ok(['tracks' => []]);
}

$entries = @scandir($sfxDir);
if (!is_array($entries)) {
    json_ok(['tracks' => []]);
}
...
foreach ($entries as $entry) {
    ...
    $fullPath = $sfxDir . DIRECTORY_SEPARATOR . $entry;
    if (!is_file($fullPath)) {
        continue;
    }
