...
(function () {
  ...
  const STAR_SHADER_WGSL = /* wgsl */`
    ...
    @vertex
    fn vs_main(in : StarAttribs) -> VSOut {
      var out : VSOut;

      let worldX = (in.pos.x + uView.panX) * uView.zoom;
      let worldY = (in.pos.y + uView.panY) * uView.zoom;
      out.clipPos = vec4<f32>(worldX / uView.aspect, worldY, 0.0, 1.0);

      var brightness = 1.0;
      var sz = 2.5;

      let selected = i32(in.index) == uView.selectedIdx;
      let hovered  = i32(in.index) == uView.hoveredIdx;
  ...
  class Galaxy3DRendererWebGPU {
    ...
    constructor(container, opts = {}) {
      ...
      this._starBuf    = null;
      this._indexBuf   = null;  // per-star u32 index for selection
      this._starCount  = 0;
      this._starScale  = 1;
      ...
      this._pipeline = this._device.createRenderPipeline({
        ...
        vertex: {
          ...
          buffers: [
            ...
            {
              // Per-star index (1 u32 = 4 bytes/vertex)
              arrayStride: 4,
              stepMode: 'vertex',
              attributes: [
                { shaderLocation: 2, offset: 0, format: 'uint32' },     // index
              ],
            },
          ],
        },
    ...
    _uploadStars(stars) {
      ...
      if (this._indexBuf) { this._indexBuf.destroy(); this._indexBuf = null; }
      ...
      // Index buffer (u32 per star)
      const idxData = new Uint32Array(this._starCount);
      for (let i = 0; i < this._starCount; i++) idxData[i] = i;
      this._indexBuf = this._device.createBuffer({
        size: idxData.byteLength,
        usage: GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST,
      });
      this._device.queue.writeBuffer(this._indexBuf, 0, idxData.buffer, 0, idxData.byteLength);
    }
    ...
    _renderFrame() {
      ...
      uni.setFloat32(24, 0, true);
      uni.setFloat32(28, 0, true);
      ...
      if (this._starBuf && this._indexBuf && this._starCount > 0 && this._bindGroup) {
        pass.setPipeline(this._pipeline);
        pass.setBindGroup(0, this._bindGroup);
        pass.setVertexBuffer(0, this._starBuf);
        pass.setVertexBuffer(1, this._indexBuf);
