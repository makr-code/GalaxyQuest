  ...
  function bindGalaxyNavOrb(root, bindRepeatButton) {
    ...
    const drawGizmo = (nowMs = (state.windowRef.performance?.now?.() || 0)) => {
      ...
      const ringDefs = (three?.Vector3 && basis)
        ? [
            { key: 'u', label: 'U', color: 'rgba(255, 200, 120, 0.95)', normal: new three.Vector3(0, 0, 1), axisProj: ez },
            { key: 'v', label: 'V', color: 'rgba(130, 255, 225, 0.95)', normal: new three.Vector3(0, 1, 0), axisProj: ey },
            { key: 'w', label: 'W', color: 'rgba(222, 160, 255, 0.95)', normal: new three.Vector3(1, 0, 0), axisProj: ex },
          ]
        : [];
